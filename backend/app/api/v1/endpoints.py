...
async def process_log(file: UploadFile = File(...), max_points: int = Query(500)):
    try:
        ...
        # 2. Парсинг (MVP: GPS + IMU) [cite: 16, 17]
        ...
        # 3. Базова обробка координат (WGS-84 -> ENU) [cite: 21]
        full_data = FlightProcessor.convert_to_local_system(gps_raw)

        # =========================================================
        # 4. ЯДРО АНАЛІТИКИ (Обов'язкова алгоритмічна частина) [cite: 18, 19]
        # =========================================================

        # А. Дистанція через Haversine
        total_dist = 0
        ...
        # Б. Швидкості через Трапецієвидне інтегрування IMU
        vx, vy, vz = 0, 0, 0
        max_h_speed = 0
        max_v_speed = 0
        max_acc = 0
        ...
        duration = imu_raw[-1]['time_s'] - imu_raw[0]['time_s']

        # =========================================================
        # 5. ФОРМУВАННЯ ВІДПОВІДІ (JSON BLOCK)
        # =========================================================
        ...
        return {
            "status": "success",
            "data": full_data[:max_points],  # Точки для Вані [cite: 20]
            "analysis": analysis_block,
            "meta": {"filename": file.filename, "status": "Ready for merge"}
        }
